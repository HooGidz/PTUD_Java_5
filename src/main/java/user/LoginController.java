 ...
 * Xử lý các nghiệp vụ liên quan đến Người dùng (Đăng nhập, Đăng ký, Đăng xuất)
 */
...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action != null && action.equalsIgnoreCase("logout")) {
			...
			response.sendRedirect("HomeCotroller"); // Chuyển hướng về trang chủ
		} else {
			// Mặc định, chuyển hướng đến trang Hồ sơ (Profile) hoặc trang chủ
			response.sendRedirect("HomeCotroller");
		}
	}
