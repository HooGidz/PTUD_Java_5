				...
				<div class="table-responsive text-nowrap">
					<table class="table">
						...
						<tbody class="table-border-bottom-0">
							<c:forEach items="${listProvider}" var="o" varStatus="status">
								<tr>
									<td>${status.count}</td>
									<td><i class="fab fa-angular fa-lg text-danger me-3"></i>
										<strong>${o.providerName }</strong></td>
									<td>${o.phone }</td>
									<c:if test="${o.status}">
										<td><span class="badge bg-label-primary me-1">Hợp
												tác</span></td>

									</c:if>

									<c:if test="${!o.status}">
										<td><span class="badge bg-label-danger me-1">Ngừng
												hợp tác</span></td>
									</c:if>
									<td>
										<div class="dropdown">
